...
def main(argv):
    ...
    parser.add_argument('--solver', default='sgd', help='Optimizer to use: Can be adam, momentum, rmsprop or sgd') 
    parser.add_argument('--num_epochs', type=int, default=1000, help='Number of training epochs')
    parser.add_argument('--batch_size', type=int, default=64, help='Batch size')
    parser.add_argument('--learning_rate', type=float, default=3e-1, help='Learning rate')
    ...
    parser.add_argument('--dim_hidden', type=int, default=2000, help='Dimension of the hidden state in each LSTM')
    parser.add_argument('--dim_embed', type=int, default=600, help='Dimension of the word embedding')
    parser.add_argument('--dim_dec', type=int, default=2000, help='Dimension of the vector used for word generation')
    parser.add_argument('--num_init_layers', type=int, default=3, help='Number of layers in the MLP for initializing the LSTMs')
    parser.add_argument('--init_lstm_with_fc_feats', action='store_true', default=True, help='Turn on to initialize the LSTMs with fc7 feats of VGG16 net. Only useful if VGG16 is used')
    ...
    parser.add_argument('--init_dec_bias', action='store_true', default=True, help='Turn on to initialize the bias for word generation with the frequency of each word')
    parser.add_argument('--class_balancing_factor', type=float, default=0.15, help='Class balancing factor. The larger it is, the model pays more attention to rare words.') 

    args = parser.parse_args()

    with tf.Session() as sess:    
	if args.phase == 'chinese_train': 
            train_data = prepare_chinese_train_data(args)
	    #print train_data 
            model = CaptionGenerator(args, 'train')
            sess.run(tf.initialize_all_variables())
            model.train_chinese(sess, train_data)
	
	elif args.phase == 'chinese_val': 
            val_data = prepare_chinese_val_data(args)
	    #print train_data 
            model = CaptionGenerator(args, 'val')
            model.load(sess)
            sess.run(tf.initialize_all_variables())
            model.val_chinese(sess, val_data)

        # testing phase
    elif  args.phase == 'chinese_test':
            test_data = prepare_chinese_test_data(args)
